  ...
  return (
    <div className="min-h-screen bg-[#f8fff6] text-[#123524]">
      ...
      <main id="top">
        ...
        {/* チーム遠征セクション */}
        <section id="team" className="relative overflow-hidden bg-gradient-to-br from-[#0b6a37] via-[#0e7d42] to-[#137744] py-20 text-white">
          <div className="absolute -right-24 -top-24 h-72 w-72 rounded-full bg-[#ffcd00]/15 blur-3xl" />
          <div className="absolute -bottom-24 -left-24 h-72 w-72 rounded-full bg-white/10 blur-3xl" />
          <div className="relative mx-auto w-full max-w-6xl px-4 sm:px-6">
            <div className="text-center">
              <p className="mb-3 inline-flex rounded-full border border-white/30 bg-white/10 px-4 py-1 text-xs tracking-widest text-white/80 uppercase">Team Expedition</p>
              <h2 className="text-2xl font-extrabold sm:text-3xl md:text-4xl">ブラジルチーム遠征・国際交流プログラム</h2>
              <p className="mx-auto mt-4 max-w-2xl text-sm leading-7 text-white/85 sm:text-base">
                ジュニアチーム・ジュニアユース・高校・大学向けに、<br className="hidden sm:block" />
                ブラジル現地でのトレーニング・練習試合・国際交流をサポートします。
              </p>
            </div>

            <div className="mt-12 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
              {[
                { icon: "⚽", label: "現地クラブとの練習試合" },
                { icon: "🏟️", label: "ブラジルクラブ訪問" },
                { icon: "🌴", label: "サントス観光" },
                { icon: "🗣️", label: "通訳サポート" },
                { icon: "🏨", label: "宿泊手配" },
                { icon: "✈️", label: "空港送迎" },
                ...
                <div key={label} className="flex items-center gap-3 rounded-2xl border border-white/20 bg-white/10 px-5 py-4 backdrop-blur-sm">
            ...
            <div className="mt-12 rounded-3xl border border-white/20 bg-white/10 p-8 backdrop-blur-sm">
              <h3 className="text-lg font-extrabold">こんなチームにおすすめ</h3>
              <ul className="mt-5 grid gap-3 sm:grid-cols-2">
                {[
                  ...
                  <li key={item} className="flex items-center gap-3 text-sm">
                    <span className="flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-[#ffcd00] text-xs font-extrabold text-[#0b6a37]">✓</span>
                    {item}
                  </li>
                ))}
              </ul>
            ...
            <div className="mt-10 text-center">
              <a
                ...
                target="_blank"
                rel="noopener noreferrer"
                className="inline-flex items-center justify-center gap-3 rounded-full bg-[#06C755] px-10 py-5 text-base font-extrabold text-white shadow-lg transition hover:bg-[#05b34c]"
              >
                <svg viewBox="0 0 24 24" className="h-5 w-5" fill="currentColor" aria-hidden="true"><path d="M19.365 9.863c.349 0 .63.285.63.631 0 .345-.281.63-.63.63H17.61v1.125h1.755c.349 0 .63.283.63.63 0 .344-.281.629-.63.629h-2.386c-.345 0-.627-.285-.627-.629V8.108c0-.345.282-.63.63-.63h2.386c.346 0 .627.285.627.63 0 .349-.281.63-.63.63H17.61v1.125h1.755zm-3.855 3.016c0 .27-.174.51-.432.596-.064.021-.133.031-.199.031-.211 0-.391-.09-.51-.25l-2.443-3.317v2.94c0 .344-.279.629-.631.629-.346 0-.626-.285-.626-.629V8.108c0-.27.173-.51.43-.595.06-.023.136-.033.194-.033.195 0 .375.104.495.254l2.462 3.33V8.108c0-.345.282-.63.63-.63.345 0 .63.285.63.63v4.771zm-5.741 0c0 .344-.282.629-.631.629-.345 0-.627-.285-.627-.629V8.108c0-.345.282-.63.63-.63.346 0 .628.285.628.63v4.771zm-2.466.629H4.917c-.345 0-.63-.285-.63-.629V8.108c0-.345.285-.63.63-.63.348 0 .63.285.63.63v4.141h1.756c.348 0 .629.283.629.63 0 .344-.281.629-.629.629M24 10.314C24 4.943 18.615.572 12 .572S0 4.943 0 10.314c0 4.811 4.27 8.842 10.035 9.608.391.082.923.258 1.058.59.12.301.079.766.038 1.08l-.164 1.02c-.045.301-.24 1.186 1.049.645 1.291-.539 6.916-4.078 9.436-6.975C23.176 14.393 24 12.458 24 10.314"/></svg>
                LINEで遠征相談する
              </a>
